...
/***/ function(module, exports, __webpack_require__) {
	...
	var DealsModel = __webpack_require__(1);
	var JsonQuery = __webpack_require__(8);
	var ko = __webpack_require__(7);
	__webpack_require__(9);
	__webpack_require__(10);
	__webpack_require__(11);

	var viewModel = new DealsModel([]);
	ko.applyBindings(viewModel);

	function setCurrentPosition(pos) {
		var geocoder, currentPosition;
		...
		viewModel.locatedAddress(null);
		geocoder = new google.maps.Geocoder();
		geocoder.geocode({ 'location': pos }, function (results, status) {
			if (status === 'OK') {
				var address = new JsonQuery(results).at(1).get('formatted_address').result();
				viewModel.locatedAddress(address);
			} else {
	...
	if (navigator.geolocation) {
		navigator.geolocation.watchPosition(
			function (position) {
				viewModel.isLocatingAddress(false);
				setCurrentPosition({
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function (deals) {
		var self = this;
		self.deals = ko.observableArray(deals);
		self.locatedAddress = ko.observable(null);
		self.currentPosition = ko.observable(null);
		self.country = ko.observable(null);
		self.isLocatingAddress = ko.observable(true);
		self.isSpinning = ko.computed(function () {
			return this.deals().length === 0;
		}, this);
		self.message = ko.computed(function () {
			if (this.locatedAddress()) {
				return 'Your location: ' + this.locatedAddress();
			}
			if (this.currentPosition()) {
				return 'Your location found!';
			}
		...
		self.search = function () {
			...
			if (currentPosition) {
				self.deals([]);
				Groupon.getDeals(currentPosition.lat, currentPosition.lng, Settings.searchRadiusMiles, function (deals) {
					var dealsVms = deals.map(function (deal) {
...
/***/ function(module, exports) {
	...
	module.exports = function () {
		...
		var searchRadiusKm = 16;

		return {
			searchRadiusKm: searchRadiusKm,
			searchRadiusMiles: searchRadiusKm / 1.6
		};
	}();
...
/***/ function(module, exports) {
	...
	module.exports = function (data) {
		var self = this, deal = data.deal;
		self.distance = data.distance;
		self.title = deal.shortAnnouncementTitle || deal.announcementTitle || deal.title || "deal";
		self.image = deal.mediumImageUrl;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var recognisedIsoCodes = ['AR', 'AU', 'BE', 'BR', 'FR', 'DE', 'IE', 'IL', 'IT', 'MY', 'NL', 'NZ', 'SG', 'ZA', 'ES', 'AE', 'GB', 'US'];

	module.exports = function () {

		function getGrouponDealsUrlForCountryCode(isoCountryCode) {

			// TODO we can include the country specific affiliate id in here
			if (isoCountryCode !== 'US' && recognisedIsoCodes.indexOf(isoCountryCode) > -1) {
				var grouponCountryCode = mapIsoToGrouponCC(String(isoCountryCode));
				return 'https://partner-int-api.groupon.com/deals.json?country_code=' + grouponCountryCode + '&';
			}

			// Default to US if country code is not recognised. US has it's own URL.
			return 'https://partner-api.groupon.com/deals.json?';
		}

		function mapIsoToGrouponCC(cc) {
			return cc === 'GB' ? 'UK' : cc;
		}
		
		// Returns the physical outlet locations 0..n for a particular deal
		function getDealLocations(deal) {
			var result = [];

			if (deal && deal.options) {
				deal.options.forEach(function (option) {
					if (option && option.redemptionLocations) {
						option.redemptionLocations.forEach(function (r) {
							if (typeof (r.lng) != 'undefined' && typeof (r.lat) != 'undefined' && r.lng && r.lat) {
								var description = r.neighborhood && r.city ? r.neighborhood + ', ' + r.city : (r.neighborhood || r.city);
								result.push({ lng: r.lng, lat: r.lat, description: description });
							}
						});
					}
				});
			...
			result = result.unique(function (loc) {
				return String(loc.lng) + ',' + String(loc.lat);
			});
			return result;
		}

		function processDeals(lat, lng, data) {
			var result = [];
			data.deals.forEach(function (deal) {
				return getDealLocations(deal).forEach(function (loc) {
					result.push({
						deal: deal,
						location: loc,
						distance: Util.distance(lat, lng, loc.lat, loc.lng)
					});
				});
			});
			result = result.filter(function (d) { return d.distance <= Settings.searchRadiusKm; });		
			return result;
		}
		
		return {
			getDeals: function (lat, lng, radius, callback) {
				var countries, i, successCallBack;

				successCallBack = function (data) {
					callback(processDeals(lat, lng, data));
				};

				countries = Util.countriesFor(lat, lng);
				...
				if (countries.length === 0) {
					...
					for (i = 0; i < countries.length; i += 1) {
						$.ajax({
							url: getGrouponDealsUrlForCountryCode(countries[i]) +
								'tsToken=' + token +
								'&lat=' + lat +
								'&lng=' + lng +
								'&radius=' + radius +
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function() {(function(n){var x=this||(0,eval)("this"),u=x.document,M=x.navigator,v=x.jQuery,F=x.JSON;(function(n){"function"===typeof define&&define.amd?define(["exports","require"],n): true?n(module.exports||exports):n(x.ko={})})(function(N,O){function J(a,c){return null===a||typeof a in T?a===c:!1}function U(b,c){var d;return function(){d||(d=a.a.setTimeout(function(){d=n;b()},c))}}function V(b,c){var d;return function(){clearTimeout(d);d=a.a.setTimeout(b,c)}}function W(a,
...
/***/ function(module, exports) {
	...
	module.exports = function () {
		...
		JsonQuery.prototype.get = function (key) {
			if (this.object) {
				return new JsonQuery(this.object[key]);
			}
			return new JsonQuery(null);
		};

		JsonQuery.prototype.map = function (f) {
			if (this.object && this.object.constructor === Array) {
				return new JsonQuery(
				this.object.map(function (item) { return f(item); })
		...
		JsonQuery.prototype.getEach = function (key) {
			return this.map(function (item) { return item[key]; });
		};

		JsonQuery.prototype.at = function (idx) {
			if (this.object) {
				return new JsonQuery(this.object[Number(idx)]);
			}
			return new JsonQuery(null);
		};

		JsonQuery.prototype.first = function () {
			return this.at(0);
		};

		JsonQuery.prototype.result = function () {
